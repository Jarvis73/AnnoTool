...
class LabelFile(object):
    ...
    def saveCommonFormat(self, filename, shapes, imagePath, imageData,
                         lineColor=None, fillColor=None, databaseSrc=None):
        imgFolderPath = os.path.dirname(imagePath)
        imgFolderName = os.path.split(imgFolderPath)[-1]
        imgFileName = os.path.basename(imagePath)
        ...
        for shape in shapes:
            ...
            if dtype == Shape.RECTANGLE:
                ...
                bndbox = LabelFile.convertPoints2BndBox(points)
                writer.addEllipseBox(bndbox[0], bndbox[1], bndbox[2], bndbox[3], label, difficult, **ext_items)
    ...
    @staticmethod
    def isLabelFile(filename):  # bug --> suffix
