...
"""
    Draw interactions with classes in this script.
"""
...
DEFAULT_BG_FILL_COLOR = QColor(0, 0, 255)


class Shape(object):
    """ Shape base class (Rectangle)
    """
    ...
    def nearPoint(self, point, epsilon):
        return distance(self.points[0] - point) <= epsilon
...
class Point(Shape):
    ...
    def drawVertex(self, path, i):
        ...
        if i == self._highlightIndex:
            ...
            d *= size
        if self._highlightIndex is not None:
            self.vertex_fill_color = self.select_fill_color
        else:
            self.vertex_fill_color = Point.vertex_fill_color
        if shape == self.P_SQUARE:
            path.addRect(point.x() - d / 2, point.y() - d / 2, d, d)
        elif shape == self.P_ROUND:
            path.addEllipse(point, d / 2.0, d / 2.0)
